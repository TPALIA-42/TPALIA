%% --- Naive AI : Take the first move available ---
%% > naiveAI/2 : +Moves, -Move
naiveAI(Moves,Move) :-
    nth1(1,Moves,Move).

%% --- Random AI : take random move ---
%% randomChoose/2 : +List, -Elt
randomChoose([], []).
...
minimax(0,Player,Board,OriginalBoard,MaxMin,_,Value) :-
    value(Player,Board,OriginalBoard,0,V),
    Value is V*MaxMin,!.

minimax(D,Player,Board,OriginalBoard,MaxMin,Move,Value) :-
    D > 0,
...
alphaBeta(D,Player,Board,OriginalBoard,Alpha,Beta,Move,Value) :-
    D > 0,
    NewPlayer is 1-Player,
    D1 is D-1,
    Alpha1 is -1*Beta,
